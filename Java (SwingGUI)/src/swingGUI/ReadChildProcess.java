...
public class ReadChildProcess {
	...
	public ReadChildProcess(GUIFrame gUIFrame) {
		this.setGUIFrame(gUIFrame);
	}
	
	public void readChildProcessStream(String path) {
		try {
		SwingGUI.getSubProcess().createSubProcess(path);
		SwingGUI.getInputOutputStream().setInputStream(SwingGUI.getSubProcess().getChildProcess().getInputStream());  // lesen die Inputstreams des Child-Prozesses
		
		String line; 
		int counter=0;

			while(SwingGUI.isChildprocessalivebool() && (line = SwingGUI.getInputOutputStream().getInputStream().readLine()) != null) {
				try {
					SwingGUI.getInputOutputStream().controlInputOutputStream(line);
