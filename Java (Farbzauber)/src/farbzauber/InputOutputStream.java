...
public class InputOutputStream {
	...
	public InputOutputStream (FarbzauberFrame gUIFrame) {
		this.setGUIFrame(gUIFrame);
	}

	public void setGUIFrame(FarbzauberFrame gf) {
		gUIFrame = gf;
	...
	public void controlInputOutputStream(String value) throws IOException, XYCoordinatesException {
		...
		setOutputStream(Farbzauber.getSubProcess().getChildProcess().getOutputStream()); // Schicken das Stream zum Kindprozess
		...
		if (value.length() == 12) { // Stringvalue enthaelt X,Y,R,G,B Werte in HEX
			...
			dp = null;
		}else if(value.length() >= 6 && value.substring(0, 7).equals("dcircle")){
		...
		}else if(value.length() >= 6 && value.substring(0, 7).equals("fcircle")){
			FillCircle fc = new FillCircle(value);
			if (fc.getXPosition() > panelCenter.getWidth() - 1 || fc.getYPosition() > panelCenter.getHeight() - 1 || fc.getXPosition() < 0 || fc.getYPosition() < 0){   
								   /*
								    * Exception werfen, wenn x und/oder y Koordinaten Ueberschritten werden
								    */
				throw new XYCoordinatesException();
		...
		}else if(value.length() >= 8 && value.substring(0, 9).equals("dtriangle")){
		...
		else if(value.length() >= 8 && value.substring(0, 9).equals("ftriangle")){
			FillTriangle ft = new FillTriangle(value);
			if (	ft.getX1Position() > panelCenter.getWidth() - 1 ||
					ft.getY1Position() > panelCenter.getHeight() - 1 ||
					ft.getX1Position() < 0 || ft.getY1Position() < 0||
					
					ft.getX2Position() > panelCenter.getWidth() - 1 ||
					ft.getY2Position() > panelCenter.getHeight() - 1 ||
					ft.getX2Position() < 0 || ft.getY2Position() < 0||
					
					ft.getX3Position() > panelCenter.getWidth() - 1 ||
					ft.getY3Position() > panelCenter.getHeight() - 1 ||
					ft.getX3Position() < 0 || ft.getY3Position() < 0){   
								   /*
								    * Exception werfen, wenn x und/oder y Koordinaten Ueberschritten werden
								    */
				throw new XYCoordinatesException();
					...
					panelCenter.addFillTriangle(ft);
			}
			
		}
		else if(value.length() >= 9 && value.substring(0, 10).equals("drectangle")){
		...
		else if(value.length() >= 9 && value.substring(0, 10).equals("frectangle")){
			FillRectangle fr = new FillRectangle(value);
			if (fr.getXPosition() > panelCenter.getWidth() - 1 ||
					fr.getYPosition() > panelCenter.getHeight() - 1 ||
					fr.getXPosition() < 0 || fr.getYPosition() < 0){   
								   /*
								    * Exception werfen, wenn x und/oder y Koordinaten Ueberschritten werden
								    */
				throw new XYCoordinatesException();
					...
					panelCenter.addFillRectangle(fr);
			}
		}
