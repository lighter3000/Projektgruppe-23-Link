...
def keyevent(ev):
    if ev.key == "Enter":
        ...
        load_linesnumbers()
    if ev.ctrlKey and ev.keyCode == 90: # Strg+Z
        print("Strg+Z")
    ev.stopPropagation()
    load_linesnumbers()
...
def insert_element(element):
    code = document["code-editor-source"]
    selection = window.getSelection()
    
    # Überprüfe, ob die Auswahl im <code>-Element liegt
    if code.contains(selection.anchorNode):
        ...
        # Füge das Element in den Text ein
        text = code.textContent
        code.textContent = text[:start_offset] + element + text[end_offset:]
        
        # Setze die Auswahl zurück
        range = document.createRange()
        range.setStart(code.firstChild, start_offset + len(element))
        range.setEnd(code.firstChild, start_offset + len(element))
        selection.removeAllRanges()
        selection.addRange(range)


def on_paste(ev):
    # Verhindern des Standard-Einfügeverhaltens
    ev.preventDefault()
    
    # Zugriff auf den Text, der eingefügt werden soll
    text = ev.clipboardData.getData("text/plain")
    
    # Einfügen des Textes an der Cursorposition
...
# Verbindet Keyevents durch Tastendruck mit der Funktion keyevent

#document["code-editor-pre"].bind("keypress", keyevent)
document["code-editor-pre"].bind("keydown", keyevent)


# Globalisiert Funktionen, damit JavaScript drauf zugreifen kann

window.insertElement = insert_element
window.loadLinenumbers = load_linesnumbers


element = document.getElementById("code-editor-source")
element.bind("paste", on_paste)